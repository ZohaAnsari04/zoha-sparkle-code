...
export const StaggeredMenu = ({
    ...
    const buildOpenTimeline = useCallback(() => {
        ...
        if (numberEls.length) {
            gsap.set(numberEls, { "--sm-num-opacity": 0 } as Record<string, number>);
        }
        if (socialTitle) {
    ...
    const handleItemClick = (e: React.MouseEvent<HTMLAnchorElement | HTMLButtonElement>, href: string) => {
        e.preventDefault();
        closeMenu();
        if (href === "#") {
    ...
    const handleResumeDownload = () => {
        window.open("https://acrobat.adobe.com/id/urn:aaid:sc:AP:33fe1638-23fb-4e96-8241-ad0b9f0e24ef", "_blank", "noopener,noreferrer");
        closeMenu();
    };
    ...
    return (
        ...
        >
            <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
                {(() => {
                    const raw = colors && colors.length ? colors.slice(0, 4) : ["#121214", "#881337"];
                    const arr = [...raw];
                    if (arr.length >= 3) {
            ...
            <header className={`staggered-menu-header ${isScrolled ? "scrolled" : ""}`} aria-label="Main navigation header">
                <button
                    ...
                    className="text-xl lg:text-2xl font-bold gradient-text hover:scale-105 transition-transform cursor-pointer whitespace-nowrap"
                >
                    {logoText}
                </button>

                <div className="flex items-center gap-3.5 sm:gap-5 z-20">
                    {/* Resume Button */}
                    <Button
                        onClick={handleResumeDownload}
                        className="bg-black/40 hover:bg-black/60 dark:bg-zinc-900/40 dark:hover:bg-zinc-900/60 backdrop-blur-md text-foreground border border-[#3b82f6]/40 hover:border-[#3b82f6]/80 shadow-[0_0_20px_rgba(59,130,246,0.25)] hover:shadow-[0_0_30px_rgba(59,130,246,0.45)] rounded-full px-5 sm:px-7 py-2.5 sm:py-3 text-xs sm:text-sm font-bold transition-all duration-300 hover:scale-105 flex items-center gap-2 whitespace-nowrap"
                    >
